...
`src/modules/ai/` contains only `.gitkeep`. `docs/next-phase/PHASE_15_AI_SKELETON.md` specifies:
...
`STATUS.md:75` says Phase 14 is built; lines 221-224 and §9 (348-367) of the
same file say the directory doesn't exist and name OpenAI/Pinecone.
`ARCHITECTURE.md:28` still lists "LangChain + LangGraph + OpenAI + Pinecone/Qdrant".
`archive/reviews/RECOMMENDED_SKILLS_ROADMAP.md:13,38` still says GPT-4o and 1536-dim vectors.
`PHASE_15_AI_SKELETON.md:110` claims `OPENAI_API_KEY` is required.
`ROUTES.md:198-215` marks all five shipped wardrobe routes "🔲 Planned".
...
- n8n ruled out (`next-phase/PHASE_15_PRODUCT_BRIEF.md:11-19`). Correct, for the stated auth reason.
...
| Conversation persistence | `next-phase/PHASE_15_PRODUCT_BRIEF.md:54-55` lists it as an open decision |
...
**Retrieval strategy = the brief's option (c), hybrid** (`next-phase/PHASE_15_PRODUCT_BRIEF.md:48-51`) —
...
| `suggestBookStylist(context)` | Optional, near-zero cost. Surfaces Murafiq's actual business when the wardrobe fails — `next-phase/PHASE_15_PRODUCT_BRIEF.md:33` already anticipates this hook. Reads `stylistService`, never a model |
...
| `AiConversation` + `AiMessage` *(extended — R3)* | Multi-turn context and debuggability | **MongoDB, not Firestore.** Firestore holds human↔human chat with rules built around booking participants; the AI conversation needs server-side reads for context assembly and has an entirely different access pattern. Do not conflate them (`next-phase/PHASE_15_PRODUCT_BRIEF.md:66-67` makes the same warning).<br>**Revision 3 adds to `AiMessage`:** `imageUrl` (temporary Cloudinary asset), `imageAnalysis` (the `garmentAnalysis` object — reused by Save, so it is never recomputed), `imageExpiresAt` (swept), `matchedWardrobeItemId` (nullable soft hint), `savedWardrobeItemId` (nullable — makes Save idempotent). **No new collection.** |
...
- **Egypt's PDPL cross-border transfer restriction.**
  `docs/archive/design/REVISION_MODERATION_CLASSIFIER_GATE.md` §3 identified exactly this issue as the
...
- **What conflicts (~10%):** two decisions in the current plan should be reversed.
  **LangGraph** (`AGENTS.md:76`, `ARCHITECTURE.md:28`) — the workflow is a pipeline
