...
> so existing references (`PHASES_INDEX.md`, `AGENTS.md`) still resolve.
>
...
## Architecture decision of record

The model strategy, RAG design, scope guard, cost model and the three revision passes that
produced them live in **[`PHASE_15_AI_ARCHITECTURE_DECISION.md`](PHASE_15_AI_ARCHITECTURE_DECISION.md)**
(previously filed under a placeholder filename). That document is the single source of truth for
*why* the architecture is shaped this way. The product framing — the *what* and *why* of the
feature itself — is in [`PHASE_15_PRODUCT_BRIEF.md`](PHASE_15_PRODUCT_BRIEF.md).

Do not restate either of them here or in `PHASE_15A`–`PHASE_15F`; amend the source document.
...
(`next-phase/PHASE_15_PRODUCT_BRIEF.md:30-33`). Answers must reference the client's real items.
