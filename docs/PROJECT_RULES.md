...
This file governs how **any** AI assistant (Claude Code, another tool, or a human following the same process) implements the phases described in this `docs/` folder. Read this before touching any phase — it's the process contract, not the technical spec (that's `ARCHITECTURE.md` and the `PHASE_XX_*.md` files).
...
- Work through phases in the order defined by `PHASES_INDEX.md`. Never start a phase whose dependency phase isn't fully done per *its own* Definition of Done checklist.
...
Follow the architectural rules in `ARCHITECTURE.md`:
