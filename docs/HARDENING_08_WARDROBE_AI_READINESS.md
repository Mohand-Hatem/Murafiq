...
3. Downgrade behaviour is already specified: **grandfather existing photos, block additions,
   never auto-delete** (`REVISION_BUSINESS_RULES_AND_ARCHITECTURE.md:1007`).
...
| File | What is wrong |
|---|---|
| `docs/01_PROJECT_STRUCTURE.md:28` | Says "LangChain + LangGraph + OpenAI + RAG (Pinecone/Qdrant)". None of that is the decision |
| `docs/01_PROJECT_STRUCTURE.md:26` | BullMQ marked "Planned (Phase 12)" — it shipped in Phase 14 |
| `AGENTS.md:73-77` | Commits to LangGraph and describes `/api/v1/ai/chat` returning 501. Neither is true |
| `docs/03_SKELETON_STATUS.md` | Self-contradictory: line 75 says Phase 14 is built, lines 221-224 say the directory does not exist, §9 (348-367) still names OpenAI/Pinecone |
| `docs/04_ROUTES.md:198-215` | All five wardrobe routes marked "🔲 Planned" though they shipped |
| `docs/RECOMMENDED_SKILLS_ROADMAP.md:13,38` | Still says GPT-4o vision, Pinecone/Qdrant, 1536-dim embeddings |
| `docs/AI_ASSISTANT_BRIEF.md:40-57` | Open decisions 1, 2 and 4 are now resolved — mark them so |
