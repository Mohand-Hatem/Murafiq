...
Following the September 2026 Full System Audit (`docs/archive/audits/AUDIT_2026_09_FULL_SYSTEM.md`), the codebase underwent a comprehensive, 7-stage remediation and simplification program (`SIMPLIFICATION_IMPLEMENTATION_PLAN_2026_09.md`):
...
| 13 | **Cron Single-Instance Guards** (`STATUS.md:73`) | **Docs Updated** | Documented that module-level boolean guards protect single-instance node processes; multi-instance cluster PM2 deployments require a Redis distributed lock (tracked in deferred backlog). |
...
| 16 | **Skeleton Status §10 Contradiction (L1)** | **Docs Updated** | Updated `STATUS.md` §10 to remove Coupons and Subscriptions from "Out of Scope", noting their full completion in Revision Stages R2/R3/R6. |
...
The documented production launch command is `npm run start:prod` ([`DEPLOYMENT_READINESS.md:86`](DEPLOYMENT_READINESS.md#L86)), which invokes `pm2 start ecosystem.config.cjs --env production` ([`package.json:18`](../package.json#L18)). The `prestart:prod` lifecycle hook fires before this command, running the duplicate-subscription check.
