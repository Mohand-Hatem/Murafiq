...
1. `docs/BUSINESS_RULES.md` (R0–R12, §E–§I)
2. `docs/archive/design/REVISION_MODERATION_CLASSIFIER_GATE.md`, `docs/archive/audits/AUDIT_2026_09_FULL_SYSTEM.md`,
   `docs/archive/remediation/REMEDIATION_2026_09_FULL_SYSTEM.md`
3. Current implementation (re-read at `file:line` for every claim below)
4. Current tests (113 files / 696 tests)
5. Older phase docs

**Companion assessment:** `docs/archive/simplification/SIMPLIFICATION_ASSESSMENT_2026_09.md`. This plan **supersedes**
...
- **Rationale comments are an asset.** When code moves, its explanatory comment moves with
  it. Do not summarise, shorten or drop the `// See docs/archive/audits/AUDIT_2026_09_FULL_SYSTEM.md
...
`STATUS.md` contradicts itself (L1) while `AGENTS.md:11-12` calls it the live
...
**Affected files.** `docs/BUSINESS_RULES.md` (a new dated
amendment section), `docs/archive/simplification/SIMPLIFICATION_IMPLEMENTATION_PLAN_2026_09.md` (this file: record
...
```bash
git add docs/BUSINESS_RULES.md docs/hardening/S0-baseline.txt docs/archive/simplification/SIMPLIFICATION_IMPLEMENTATION_PLAN_2026_09.md
...
`docs/ROUTES.md`. *Test impact:* new integration test. *Rollback:* revert. *Risk:* **LOW.**
...
**must** be noted in `docs/ROUTES.md` and the Swagger description for the route.
...
> `docs/ROUTES.md` and the actual product flows **before** implementing — a wrong row here
...
- [x] **Step 1: Close every row of the audit's §9 "code contradicts the docs" table.** For
  ...
  `STATUS.md` §10 vs §83-140 (L1) · Socket.IO in 8 files (L2/P7 — recommendation:
...
- [x] **Step 2: Amend `BUSINESS_RULES.md`** — record that
...
- [x] **Step 5: Re-run the full-system audit** against the simplified tree using
  `docs/archive/audits/AUDIT_2026_09_FULL_SYSTEM.md`'s own methodology. **Acceptance: no new CRITICAL or
  HIGH.** Recorded the result as `docs/archive/audits/AUDIT_2026_09_POST_SIMPLIFICATION.md`.
...
| **K7** | S4.4's upload role matrix forbids a folder/role pair a real flow needs | Regression | MEDIUM | MEDIUM — a user journey breaks with a 403 | The matrix must be confirmed against `docs/ROUTES.md` and the real flows **before** implementing; the note in S4.4 calls this out explicitly | LOW |
