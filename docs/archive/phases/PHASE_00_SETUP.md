...
Create every folder from `ARCHITECTURE.md` under `src/`, even the ones that stay empty until later phases (`modules/ai`, `modules/payouts`, etc). Empty folders can hold a `.gitkeep`.
...
```
Mount this before `routes` so it's live even with zero business routes yet (the spec will just be near-empty until Phase 1 adds its first `@swagger` blocks). Every phase from here on adds its own annotations as it writes routes — see `ARCHITECTURE.md`.
