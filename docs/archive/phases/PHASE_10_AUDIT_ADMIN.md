...
- `GET /admin/users/:userId/subscription` — the target user's live subscription, resolved plan, entitlements and daily usage.
- `POST /admin/users/:userId/subscription` — **manual plan grant, upgrade, downgrade or revoke.** Body: `{ planCode, billingCycle?, durationDays?, reason }`. This is an *administrative entitlement operation, not a customer payment*: it must never call `provider.factory.js#getProvider()`, never create a `Payment` or `SubscriptionOrder`, and — most importantly — **never post a ledger entry**. A comp that mints a balanced DEBIT/CREDIT pair books revenue nobody paid AND reconciles cleanly, so the nightly sweep would never alert on it. It writes through the same `subscriptionService.applyPlanGrant()` primitive the Paymob webhook uses, so the entitlements a granted plan produces are identical to a purchased one by construction. Emits `SUBSCRIPTION_ADMIN_GRANTED` (in `AUDIT_EVENT_MAP`, severity `warn`) — deliberately *not* `SUBSCRIPTION_ACTIVATED`, which means "a customer paid", so a comp stays distinguishable from revenue in the audit trail. The grant **replaces** the current period outright rather than stacking onto it, and a `*.free` target revokes a paid tier **immediately**, bypassing the customer-facing scheduled-downgrade rule. Admin only — not operator.
- `GET /admin/users/:userId/subscription/history` — append-only `SubscriptionHistory`, one row per plan transition (paid, admin grant, self-service switch, scheduled downgrade, expiry sweep) carrying the state each one replaced.
