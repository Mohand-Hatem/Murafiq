...
**The defect.** `ARCHITECTURE.md` §2 lists "File Storage: Cloudinary + Multer — **Active**".
...
2. Build `src/modules/uploads/` per `ARCHITECTURE.md` §4: `upload.routes.js`,
   `upload.controller.js`, `upload.service.js`, `multer.middleware.js`. Follow the standard layering.
3. `POST /api/v1/uploads/:folder` — authenticated, folder allow-listed (per `ROUTES.md`), MIME-type
...
The only signal is one `warn` line at boot. `STATUS.md` §2a meanwhile marks all of this
"✅ Active".
...
3. Add `firebase: 'connected' | 'unavailable'` to the `/api/v1/health` response (see `HARDENING_04`
   Step 5, which also adds the missing Redis check `ROUTES.md` already claims exists).
...
3. Longer term this belongs on a queue — `STATUS.md` §4 already lists "Mail sending via
...
**The defect.** `src/modules/audit-log/` contains only a `.gitkeep`. `ARCHITECTURE.md` §4
specifies the full module (`audit-log.service.js`, `.repository.js`, `.model.js`, `.listener.js`) and
`ROUTES.md` lists `GET /admin/audit-logs`. None of it exists.
...
- **Admins can read any chat conversation with no record.** `chatService.getMessages`
  ...
  that a dispute exists on that booking, no time limit, no log. `STATUS.md` §2a describes
...
4. Add `GET /admin/audit-logs` (admin only, `QueryBuilder`-backed) per `ROUTES.md`.
...
authentication. `ROUTES.md` explicitly specifies `GET /docs` as "🔓 (or 🛡️ **in prod**)" — the
...
1. Either skip the Swagger mount when `NODE_ENV === 'production'`, or put it behind
   `authMiddleware + restrictTo(ROLES.ADMIN)`. Pick one and record which in `ROUTES.md`.
...
- [ ] Multi-device decision recorded in this file (or `ARCHITECTURE.md`) and implemented, or explicitly deferred with a reason.
