...
> Section A's questions and **stop**, exactly as `PROJECT_RULES.md` requires — these are business
...
`ROUTES.md` lists five payout routes and `ARCHITECTURE.md` §4 specifies the full module —
none of it exists.
...
1. **Payout rail.** Manual bank transfer with an admin marking batches paid (what `ROUTES.md`'s
...
no resolve endpoint (`ROUTES.md` lists `PATCH /admin/bookings/:id/resolve-dispute` — not built), no
...
Build `src/modules/payouts/` per `ARCHITECTURE.md` §4 and the five routes in `ROUTES.md`.
