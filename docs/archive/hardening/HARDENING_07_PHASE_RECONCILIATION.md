...
phase normally per `PROJECT_RULES.md`.
...
`package.json` declares `"type": "module"`, and `PROJECT_RULES.md` states **"ESM only — `import`/
...
`PHASES_INDEX.md` states the mail signature **"must stay identical"** so Phase 9 is a drop-in
...
same pass (see `STATUS.md` §7 for the full list of what changed and why). `PHASE_13`
...
`STATUS.md` §4 used to claim otherwise) and is **fixed**, not merely planned. It doesn't
...
> See `docs/next-phase/PHASE_15_PRODUCT_BRIEF.md` for the product framing (business logic, occasion-matching
...
4. **No vector DB or OpenAI package is installed** — `STATUS.md` §9 confirms this.
   Phase 14 installs them for the first time. Budget for choosing between Pinecone and Qdrant — the
   spec never decides (see `docs/next-phase/PHASE_15_PRODUCT_BRIEF.md` open decision #1).
...
2. **The dependency claim is circular and currently false.** `PHASE_15` Step 5 states *"`OPENAI_API_KEY`
   ...
   *"the vector DB SDK … installed and required as of Phase 14"*. Phase 14 has **not** been built, so
   ...
   before Phase 14 is complete.** `PHASES_INDEX.md` already encodes this dependency; the point is
   that `STATUS.md` claims both are already active, which would mislead anyone starting
   here.
...
- [ ] The upload-architecture contradiction between `PHASE_09` Step 4 and `ROUTES.md` is resolved and recorded.
