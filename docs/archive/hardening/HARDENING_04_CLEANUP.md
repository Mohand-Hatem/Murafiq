...
2. Record the convention in `PROJECT_RULES.md` under "Established conventions" so new modules
   inherit it.
...
- `ROUTES.md` documents `GET /health` as "Health check (**Mongo + Redis** status)".
...
1. Report per-dependency status in `/health`: Mongo, Firebase (per `HARDENING_03` Step 3), and Redis
   ...
   from `ROUTES.md` — don't leave the doc asserting a check that doesn't happen.
