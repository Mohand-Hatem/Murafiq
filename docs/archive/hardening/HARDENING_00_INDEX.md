...
`PHASES_INDEX.md` equivalent for repair work rather than feature work.
...
Follow `PROJECT_RULES.md` exactly — it already defines the contract:
...
problem `PHASES_INDEX.md` already warns about.
...
- `cloudinary`, `multer`, `bullmq`, and every AI/vector-DB package are **not installed**, despite
  `ARCHITECTURE.md` and `STATUS.md` marking several of them "Active".
