...
Follow `PROJECT_RULES.md`: one step at a time, present **What / Why this solution not another / How**,
...
```
Read docs/archive/hardening/HARDENING_01_CRITICAL.md fully.
Implement its "Steps" section in order, following docs/PROJECT_RULES.md
...
**only** path that ever executes, and the atomicity `ARCHITECTURE.md` principle 7 advertises
...
`DEPLOYMENT_READINESS.md` deployment options (Railway, Render) and the Nginx path put the app
...
4. Document it in `PHASE_00_SETUP.md` and in the deployment steps of `DEPLOYMENT_READINESS.md`
...
`PROJECT_RULES.md`. Do not mark from memory of having written the code.
