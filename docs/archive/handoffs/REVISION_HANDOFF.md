...
> `docs/BUSINESS_RULES.md`. This file tells the implementer *how to work*,
...
5. **`npm test` must pass before you touch anything.** The current baseline is **76 suites / 381
   ...
   starting point. Confirm the live number against `docs/STATUS.md`, not this line.
...
- **Modular monolith only.** No microservices. No new datastore. No Redis/BullMQ — that is
  deliberately deferred to Phase 14 (`docs/archive/hardening/HARDENING_07_PHASE_RECONCILIATION.md`). If a design seems
...
- **One approved step at a time.** Present What / Why (with a named alternative) / How, then **stop
  and wait for explicit approval**. Never bundle unrelated changes. This is `docs/PROJECT_RULES.md`
...
| 2 | `docs/PROJECT_RULES.md` | The approval-gated process you must follow. |
| 3 | `docs/STATUS.md` | **The only trustworthy statement of what is actually built.** Trust it over every other doc — including this one. It records the defects found in each Revision pass and which stages are genuinely complete. |
| 4 | `docs/BUSINESS_RULES.md` | **The specification.** Sections A–R. Your source of truth for this phase. |
| 5 | `docs/ARCHITECTURE.md` | File locations and naming. |
...
`docs/BUSINESS_RULES.md`** — it is the authoritative table. Summary:
...
(the history is in `docs/STATUS.md`). A previous version turned retryable transience into
...
> both phases ✅ in `docs/STATUS.md` as part of their Definition of Done.
>
...
8. **Update `docs/STATUS.md`** — this is part of Definition of Done, not cleanup.
...
- [ ] `docs/STATUS.md` updated to reflect reality.
...
1. Confirmation that you have read `AGENTS.md`, `docs/PROJECT_RULES.md`,
   `docs/STATUS.md`, and `docs/BUSINESS_RULES.md` in full.
