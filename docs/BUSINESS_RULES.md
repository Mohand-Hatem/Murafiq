...
> **This document is the source of truth for Murafiq's business rules.** Where any other
> document disagrees with it, this one wins.
>
> **Design records.** Two features specified here were designed in standalone documents before
> they shipped. The *rules* live here (§F.1/§F.2 for requests and offers, §B.6/§C.8 for the
> moderation surface); the *design rationale* is preserved, unmerged, so this file stays
> readable:
> - [`archive/design/OPEN_BROADCAST_REQUESTS_DESIGN.md`](archive/design/OPEN_BROADCAST_REQUESTS_DESIGN.md)
> - [`archive/design/REVISION_MODERATION_CLASSIFIER_GATE.md`](archive/design/REVISION_MODERATION_CLASSIFIER_GATE.md)
>
> Do not treat either as current status.
...
The 4-point gate is analysed in full in **`docs/archive/design/REVISION_MODERATION_CLASSIFIER_GATE.md`**. Summary of
...
**Related docs:** `docs/STATUS.md` (what is actually built today) · `AGENTS.md` (invariants — §J and §G.1 below propose two explicit amendments to it) · `docs/MONEY_AND_LEDGER.md` (contains a policy the code has never implemented — see §B.5).

---

## Context

Murafiq is a live, working modular monolith (Phases 0–13 built, per `docs/STATUS.md`). The product owner has issued a new set of business requirements covering chat trust & safety, dual subscription systems, request/offer lifecycle changes, a redesigned cancellation/refund/penalty model, no-show handling, a stylist reliability system, and coupons.
...
Phases 12 and 13 are both marked partial in `docs/STATUS.md`. **Their leftovers are not
...
`docs/STATUS.md`** — updating that file is part of the Definition of Done for both stages.
...
3. **Establish the baseline** before any change:
   ...
   npm test
   ```
   Expected: 51 suites / 214 tests passing, per `docs/STATUS.md`. Any deviation means the documented state is already stale and must be re-verified first.
...
5. **Approve the three invariant amendments** explicitly — each contradicts a line in `AGENTS.md`, and each must be updated there in the same commit that changes the behaviour:
   ...
   `docs/STATUS.md` was created to fix.
