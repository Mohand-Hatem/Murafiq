...
>
> **Read alongside** `hardening/POST_PHASE_15_HARDENING_BACKLOG.md` — this audit
...
- **Conflicts with:** `03_SKELETON_STATUS.md` — *"two human sign-offs remain before the moderation enforcement cutover."* The cutover is a code change, not a sign-off.
...
> change was made for this item. See `docs/REMEDIATION_2026_09_FULL_SYSTEM.md` §12.
...
- **Location:** `AGENTS.md:181-184`, `docs/MONEY_AND_LEDGER.md:42`, `docs/PHASE_06_PAYMENTS.md:73,161` vs `src/common/constants/statuses.constant.js`
...
| Crons have "single-instance guards" (`03_SKELETON_STATUS.md:73`) | Process-local flags, not locks |
...
- **`03_SKELETON_STATUS.md` contradicts itself** — §10 declares Coupons and Subscription plans "not present anywhere, and intentionally so", while §83-140 of the same file documents the subscription endpoints in detail (L1). This is the file `AGENTS.md` designates as the live source of truth.
...
3. **`HARDENING_08` #3 and #5** (enum constraints, sub-category). `00_PHASES_INDEX.md:24` declares `HARDENING_08` a hard blocker on all of Phase 15, and #3 is the silent-data-corruption class that Phase 15 would amplify — wardrobe retrieval is a Mongo slot query, so an unconstrained `formality` value removes an item from every filtered query with no error.
