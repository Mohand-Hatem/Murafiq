...
> [!NOTE]
> **Business Rules Revision — Stages R0 through R12: 100% COMPLETE & VERIFIED.** `BUSINESS_RULES.md`
...
> `ARCHITECTURE.md` §2 marks Cloudinary+Multer and BullMQ+Redis as "Active". **They are not
...
| Deployment target | ✅ **Decided (this pass)** | Manually-provisioned VPS, PM2 (`ecosystem.config.cjs`, `fork` mode, `instances: 1`), nginx reverse proxy. No Docker, no managed PaaS. See `DEPLOYMENT_READINESS.md`. |
